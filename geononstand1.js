...
// var  colorDensity = d3.scaleLinear()
//         .domain(d3.extent(states, d => d.emp ))
//         .range(["red", 'green']);
...
console.log(d3.extent(states, d => d.emp))

fetch('./atheletes.json')
.then(function(resp){
    return resp.json();
})
.then(function(data){
...
function createStates(statedata){
    ...
    function update() { 
        ...
        .on('mouseover', function(d){
            ...
            d3.select("#maintext")
                .attr('font-size', '15')
                .text(d.properties["NAME"] + " " + stateTotal[0].emp);
        })
    ...
    let stateTotal =  states.filter(function(item){    
        ...
        return item.name == stateName;                                       
            ...
            if(stateTotal[0] ==undefined){
                ...
                console.log(stateTotal[0].emp)
                return colorDensity(stateTotal[0].emp);
            }
